(function () {
  ...
  async function init(map, opts) {
    ...
    async function load() {
      ...
      if (!resp.ok) {
        // 404 = no corridor data for this home, not an error
        if (resp.status === 404) {
          console.log("Corridors: No data for this home (404)");
          allFeatures = [];
          return;
        }
        throw new Error("Failed to load corridors GeoJSON: HTTP " + resp.status);
      }
