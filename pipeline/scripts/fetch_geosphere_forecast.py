...
def parse_feature_forecasts(feature: dict, timestamps: list) -> list:
    ...
    # WICHTIG: snow_acc und rr_acc sind KUMULATIVE Werte!
    snow_acc = parameters.get('snow_acc', {}).get('data', [])   # kg/m² ≈ mm (accumulated)
    snow_limit = parameters.get('snowlmt', {}).get('data', [])  # m
    temp = parameters.get('t2m', {}).get('data', [])            # °C
    precip_acc = parameters.get('rr_acc', {}).get('data', [])   # mm (accumulated)

    forecasts = []
    prev_snow_acc = 0.0  # Track previous cumulative value
    prev_precip_acc = 0.0

    for i, ts in enumerate(timestamps):
        # Get current cumulative values
        curr_snow_acc = snow_acc[i] if i < len(snow_acc) and snow_acc[i] is not None else prev_snow_acc
        curr_precip_acc = precip_acc[i] if i < len(precip_acc) and precip_acc[i] is not None else prev_precip_acc

        # Calculate HOURLY values from cumulative difference
        snowfall_mm = max(0, curr_snow_acc - prev_snow_acc)
        precip_mm = max(0, curr_precip_acc - prev_precip_acc)

        # Update previous values for next iteration
        prev_snow_acc = curr_snow_acc
        prev_precip_acc = curr_precip_acc

        # Estimate snow cm from water equivalent
        # Ratio depends on temperature, typically 1:10 to 1:20
        snowfall_cm = None
        if snowfall_mm > 0:
            temp_at_time = temp[i] if i < len(temp) and temp[i] is not None else 0
        ...
        forecasts.append({
            'timestamp': ts,
            'snowfall_mm': round(snowfall_mm, 1) if snowfall_mm > 0 else None,
            ...
            'precip_mm': round(precip_mm, 1) if precip_mm > 0 else None,
        })
